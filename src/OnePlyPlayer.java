 ...
 * The heuristic is relatively simple:
 ...
 *   (necessary exploration, since we can't look ahead to make intelligent moves and will
      typically otherwise get stuck in a loop once we get off the start positions)
 */

import java.util.Hashtable;
public class OnePlyPlayer implements Game.Player
{
    static final int MAXMOVEDISTANCE = 2;
    static final int BOARDSIZE = 13;

    // Heuristic weights
    static final int STARTPIECEVAL = -3; // value for a piece being in a start location
    static final int WINVAL = 10; // value for winning the game
    static final int ENEMYWINVAL = -5; // value for enemy winning the game
    static final int REVISITVAL = -1; // value for visiting a configuration again
    ...
    public int heuristic(int[] m, int pID, int[] red, int[] green, int[] blue)
    {
      int val = 0;
    ...
    public int[] move(int[] red, int[] green, int[] blue, int player, int time)
    {
      ...
      int bestval = -100; // Arbitrary small starting value
      ...
      for(int i=0; i<8; i+=2)
      {
        ...
        for(int k=1; k<=MAXMOVEDISTANCE; k++)
        {
          ...
          for(int m=0; m < 6; m++)
          {
            ...
            {
              int val = heuristic(curmove,player,red,green,blue);
              if(val > bestval)
              {
